...
from app.problemset.clients.spring_client import spring_get
from app.problemset.services.dataset_analysis_service import analyze_csv_dataset


def analyze_dataset(dataset_url: str, data_file_name: str | None = None) -> dict[str, Any]:
    """
    Signed URL 또는 허용된 로컬 경로로 접근 가능한 CSV 데이터셋을 분석한다.

    관리자가 "이 데이터셋으로 문제를 만들어줘", "어떤 컬럼을 쓰면 좋을까", "파일에 맞게 문제를 구성해줘"라고 요청하면 사용한다.
    함수 결과의 컬럼명, 샘플 값, 인코딩 정보는 문제 생성 시 반드시 참고해야 한다.
    """
...
def get_problem_generation_policy() -> dict[str, Any]:
    ...
    문제세트 title, description, difficulty, problems, testCases 구조와 테스트 코드 작성 규칙을 확인할 때 사용한다.
    """
    return {
        "problem_set_schema": {
            "title": "문제세트 제목",
...
def get_similar_problem_examples(
    ...
    비슷한 주제나 난이도의 문제 스타일을 참고하되, 기존 문제를 그대로 복사하지 않도록 하기 위해 사용한다.
    1차 구현에서는 Spring 문제세트 목록 API를 사용하고, 추후 ChromaDB 같은 벡터 DB 검색으로 교체할 수 있다.
    """
    params: dict[str, Any] = {
        "page": 0,
        "size": 5,
    ...
    response["search_hint"] = {
        ...
        "topic": topic,
        "note": "topic은 현재 Gemini가 결과 해석 힌트로만 사용한다.",
    }
    return response
